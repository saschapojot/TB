...
def FindAtomSymmetry(ParaIn,ParaSym,ParaNbr):
    ...
    LvAtAtInd    = ParaNbr["LvAtAtInd"]# one entry of LvAtAtInd is [iAt,iLvAt], iAt: index of atom in 0,0,0 cell; iLvAt: row number of LvAt, taking values from 0 to NumLv*NumAt-1
    ...
    SymLvAtAtInd = np.zeros((NumSym,NumLvAtAt),int)# a symmetry operator acts on a hopping relation
    for iSym in range(NumSym):
    ...
    # Output
    ParaSymAt = {"SymLvAtAtInd": SymLvAtAtInd,#symmetry operator acts on a hopping and obtains another hopping
                 "LvAtAtClas":   LvAtAtClas,# each class is an orbit of an operation, each element in orbit is a linear combination of hopping term
                 #first atom at (0,0,0), second at Lv
                 "SymAt0":       SymAt0, # Used in CheckH, operation on atom in (0,0,0) cell
                 }
